...
#pragma mark CDA
        if( [[[im valueForKey:@"modality"] lowercaseString] isEqualToString:@"doc"] || [DCMAbstractSyntaxUID isCDA: [im valueForKeyPath: @"series.seriesSOPClassUID"]])
        {
            NSString *path = nil;
            
                DCMObject *dcmObject = [DCMObject objectWithContentsOfFile: [im valueForKey: @"completePath"] decodingPixelData:NO];
                
                if ([[dcmObject attributeValueWithName:@"SOPClassUID"] isEqualToString:[DCMAbstractSyntaxUID cdaStorageClassUID]])
                {
                    NSMutableData *cdaData = [NSMutableData dataWithData:[dcmObject attributeValueWithName:@"EncapsulatedDocument"]];
                    char lastByte;
                    NSRange lastByteRange=NSMakeRange(cdaData.length-1,1);
                    [cdaData getBytes:&lastByte range:lastByteRange];
                    if (lastByte==0) [cdaData replaceBytesInRange:lastByteRange
                                                        withBytes:NULL
                                                           length:0];
                    
                    NSString *filename = [dcmObject attributeValueWithName:@"DocumentTitle"];
                    if( [filename length] <= 0)
                        filename = @"cdaFile.xml";
                    
                    if( [[[filename pathExtension] lowercaseString] isEqualToString: @"xml"] == NO)
                        filename = [filename stringByAppendingPathExtension: @"xml"];
                    
                    path = [self.database.tempDirPath stringByAppendingPathComponent:filename];
                    [[NSFileManager defaultManager] removeItemAtPath: path error: nil];
                    [cdaData writeToFile: path atomically: YES];
                }
